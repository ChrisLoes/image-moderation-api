...
class TestMultipleApiKeys:
    ...
    def test_valid_dev_api_key(self):
        ...
        response = client.post(
            "/faces/blur",
            headers={"X-API-Key": "wievoll-dev-2324032lkADSJfk2l3"},
    ...
    def test_valid_prod_api_key(self):
        ...
        response = client.post(
            "/faces/blur",
            headers={"X-API-Key": "wievoll-prod-fsalkjWkdWlmcVje2ß927"},
            files={"file": ("test.png", MINIMAL_PNG, "image/png")},
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_all_valid_keys_in_settings(self):
        ...
        assert "wievoll-dev-2324032lkADSJfk2l3" in valid_keys
        assert "wievoll-prod-fsalkjWkdWlmcVje2ß927" in valid_keys
